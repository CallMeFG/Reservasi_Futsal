...
public class FutsalController {
    ...
    public boolean tambahInventaris(Inventaris inventaris) {
        String sql = "INSERT INTO inventaris (inventaris_id, nama_barang, jumlah) VALUES (?, ?, ?)";
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, inventaris.getInventarisId());
            pstmt.setString(2, inventaris.getNamaBrg());
            pstmt.setString(3, inventaris.getJumlah());
            return pstmt.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
    ...
    public boolean updateInventaris(Inventaris inventaris) {
        System.out.println(inventaris.getNamaBrg()+", "+inventaris.getJumlah());
        String sql = "UPDATE inventaris SET nama_barang = ?, jumlah = ? WHERE inventaris_id = ?";
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, inventaris.getNamaBrg());
    ...
    public boolean hapusInventaris(String inventarisId) {
        String sql = "DELETE FROM inventaris WHERE inventaris_id = ?";
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, inventarisId);
            return pstmt.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
    ...
    public List<Inventaris> getAllInventaris() {
        List<Inventaris> daftarInventaris = new ArrayList<>();
        ...
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql); ResultSet rs = pstmt.executeQuery()) {
            ...
            while (rs.next()) {
                String id = rs.getString("inventaris_id");
            ...
            System.err.println("Gagal mengambil data inventaris: " + e.getMessage());
            e.printStackTrace();
        }
